...
def privmsg(sender, headers, message):
    if message.startswith("!say "):
        ...
        if sender == owner:
            bot.identify(sender, saySuccess, (message[5:firstSpace], message[firstSpace + 1:]), authFailure,
                         (headers[0], sender))
        ...
        if sender == owner:
            bot.identify(sender, kickSuccess,
                         (message[6:firstSpace], message[firstSpace + 1:secondSpace], message[secondSpace + 1:]),
                         authFailure, (headers[0], sender))
    else:
        print "PRIVMSG: \"" + message + "\""


def actionmsg(sender, headers, message):
    print "An ACTION message was sent by " + sender + " with the headers " + str(
        headers) + ". It says: \"" + sender + " " + message + "\""
